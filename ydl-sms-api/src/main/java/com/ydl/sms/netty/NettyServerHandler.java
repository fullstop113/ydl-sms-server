...
public class NettyServerHandler extends SimpleChannelInboundHandler<String> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, String msg) {
        ...
        try {
            ...
            //2.dto不为空 正式调用service
            SmsSendService bean = SpringUtils.getBean(SmsSendService.class);
            bean.send(smsParamsDTO);
        } catch (Exception e){
            log.error("使用netty发送的时候出错了",e);
            res=e.getMessage();
        }
        log.info("=========回推报文：",res);
